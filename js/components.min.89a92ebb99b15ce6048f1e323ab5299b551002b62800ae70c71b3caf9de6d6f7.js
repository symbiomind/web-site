(function(){window.dashboardPages=window.dashboardPages||[];const e={authVerified:!1,pages:[],navItems:[],pageCallbacks:{},currentPageId:null,container:null,nav:null,content:null,dialogEl:null,SECTION_STATE_KEY:"dashboardSectionStates",getTabFromURL(){const e=new URLSearchParams(window.location.search);return e.get("tab")},updateURL(e){const t=new URL(window.location);t.searchParams.set("tab",e),window.history.pushState({},"",t)},getSectionStates(){try{const e=sessionStorage.getItem(this.SECTION_STATE_KEY);return e?JSON.parse(e):{}}catch{return{}}},saveSectionState(e,t){try{const n=this.getSectionStates();n[e]=t,sessionStorage.setItem(this.SECTION_STATE_KEY,JSON.stringify(n))}catch{}},findParentSection(e){for(const t of this.navItems)if(t.type==="section"&&t.children)for(const n of t.children)if(n.id===e)return t.id;return null},registerPage(e){this.navItems.push(e),e.type==="section"&&e.children?e.children.forEach(e=>{this.pages.push(e)}):this.pages.push(e),this.nav&&this.content&&(this.addNavItem(e),e.type==="section"&&e.children?e.children.forEach(e=>this.addContentPane(e)):this.addContentPane(e))},async init(e){if(this.container=document.querySelector(e),!this.container){console.error(`Dashboard container '${e}' not found.`);return}if(window.SymbioAuth){const e=await window.SymbioAuth.requireAuth();if(!e)return;this.authVerified=!0}else console.warn("SymbioAuth not available - ensure auth.js loads before dashboard");this.container.innerHTML=`
    ...
    `}).join("")}function d(){const t=document.getElementById("pane-profile");if(!t)return;const p=t.querySelector("#profile-loading"),y=t.querySelector("#profile-content"),g=t.querySelector("#profile-error"),v=t.querySelector("#profile-error-message"),x=t.querySelector("#profile-email"),O=t.querySelector("#profile-tier"),j=t.querySelector("#profile-verified"),w=t.querySelector("#profile-member-since"),_=t.querySelector("#profile-last-login"),u=t.querySelector("#profile-session-badge"),b=t.querySelector("#profile-session-list"),o=t.querySelector("#logout-all-btn");p.style.display="block",y.style.display="none",g.style.display="none",Promise.all([i(),r()]).then(([o,i])=>{if(!o)return;const a=o.user,r=o.active_sessions,d=i?i.sessions:[],c=t.querySelector("#email");c&&(c.value=a.email),x.textContent=a.email,O.textContent=s(a.subscription_tier),a.email_verified?j.innerHTML='<span class="status-verified">✓ Verified</span>':j.innerHTML='<span class="status-unverified">⚠ Not Verified</span>',w.textContent=n(a.created_at),_.textContent=e(a.last_login),r===1?(u.className="session-badge session-badge-ok",u.textContent="1 device"):(u.className="session-badge session-badge-warning",u.textContent=`${r} devices`),b&&(b.innerHTML=l(d)),p.style.display="none",y.style.display="block"}).catch(e=>{p.style.display="none",v.textContent=e.message||"Failed to load profile",g.style.display="block"}),o&&!o.dataset.initialized&&(o.dataset.initialized="true",o.addEventListener("click",async function(e){e.preventDefault();const t=confirm(`This will log you out of all devices including this one.

Are you sure you want to continue?`);if(!t)return;o.disabled=!0,o.textContent="Logging out...";try{const e=await c();e&&(sessionStorage.removeItem("accessToken"),sessionStorage.removeItem("refreshToken"),sessionStorage.removeItem("username"),window.location.href="/dashboard/login/?message=logged-out")}catch(e){v.textContent=e.message||"Failed to log out of all devices",g.style.display="block",o.disabled=!1,o.textContent="Log Out All Devices"}}));const f=t.querySelector("#change-password-form"),d=t.querySelector("#password-error"),m=t.querySelector("#password-error-message"),h=t.querySelector("#change-password-btn");f&&!f.dataset.initialized&&(f.dataset.initialized="true",f.addEventListener("submit",async function(e){e.preventDefault(),d.style.display="none";const s=t.querySelector("#current-password").value,n=t.querySelector("#new-password").value,o=t.querySelector("#confirm-password").value;if(!s||!n||!o){m.textContent="All fields are required",d.style.display="block";return}if(n!==o){m.textContent="New passwords do not match",d.style.display="block";return}if(n.length<6){m.textContent="New password must be at least 6 characters",d.style.display="block";return}h.disabled=!0,h.textContent="Changing...",t.querySelector("#current-password").disabled=!0,t.querySelector("#new-password").disabled=!0,t.querySelector("#confirm-password").disabled=!0;try{const e=await a(s,n);e&&(sessionStorage.removeItem("accessToken"),sessionStorage.removeItem("refreshToken"),sessionStorage.removeItem("username"),window.location.href="/dashboard/login/?message=password-changed")}catch(e){m.textContent=e.message||"Failed to change password",d.style.display="block",h.disabled=!1,h.textContent="Change Password",t.querySelector("#current-password").disabled=!1,t.querySelector("#new-password").disabled=!1,t.querySelector("#confirm-password").disabled=!1}}))}window.dashboardPages.push({id:"profile",title:"User Profile",html:`
  ...
  `}),window.addEventListener("dashboardReady",function(){window.dashboardApp.onPageVisible("profile",d)})}(),function(){window.dashboardPages=window.dashboardPages||[];function n(e){return e?e.split("-").join(" "):"Unknown"}function e(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}function s(e){return e?new Date(e).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):"Unknown"}async function t(){const e=document.getElementById("roles-list");if(!e)return;e.innerHTML='<p class="loading-text">Loading roles...</p>';try{const n=sessionStorage.getItem("accessToken"),t=await fetch(`${SymbioConfig.api_url}/api/roles/list`,{method:"GET",headers:{Authorization:`Bearer ${n}`}});if(!t.ok)throw new Error(`API returned ${t.status}`);const s=await t.json();a(e,s.roles)}catch(s){console.error("Failed to load roles:",s),e.innerHTML=`
    ...
    `}function a(t,s){if(!s||s.length===0){t.innerHTML=`
        <p class="empty-state">No roles found. Create a namespace to get started!</p>
      `;return}const r=o(s);let a="";Object.keys(r).sort().forEach(t=>{const o=n(t),s=r[t];a+=`
        <div class="profile-card">
          <h3>${e(o)}</h3>
      `,Object.keys(s).sort().forEach(t=>{const n=s[t],o=n.roles.map(e=>i(e)).join("");a+=`
          <div class="resource-group">
            <h4 class="resource-subheader">${e(t)}</h4>
            ${o}
          </div>
        `}),a+=`</div>`}),t.innerHTML=a}function r(){const e=document.getElementById("pane-roles");if(!e)return;t()}window.dashboardPages.push({id:"roles",title:"Roles",html:`
      <div class="page-header">
        <h2>Roles</h2>
    ...
    `}),window.addEventListener("dashboardReady",function(){window.dashboardApp.onPageVisible("roles",r)})}(),function(){window.dashboardPages=window.dashboardPages||[],window.dashboardPages.push({id:"memory-mcp",title:"Memory MCP",type:"section",expanded:!0,children:[{id:"memory-mcp-resources",title:"Namespaces",html:`
          <div class="page-header">
            <h2>Namespaces</h2>
      ...
      `;return}const s=n.map(e=>{const n=new Date(e.created_at).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}),s=e.embedding_config_id?"Custom":"System Default";return`
        <div class="item-card" data-namespace-id="${e.id}">
          <div class="item-info">
            <h4 class="item-name">${t(e.friendly_name)}</h4>
            <div class="item-meta">
